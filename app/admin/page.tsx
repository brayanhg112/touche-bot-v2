...
export default function AdminPage() {
  ...
  const OPTIONS = useMemo(() => ({
    ...
    olfactory_family: ["FLORALES", "ORIENTALES/ESPEC", "DULCES", "AROMATICO", "FRUTALES", "FRESCO", "ATALCADO", "ACUATICAS"],
    occasion: ["OFICINA", "USO DIARIO", "CITA ROMANTICA", "EVENTO FORMAL", "DEPORTE"],
  ...
  // ── Persist inventory ──────────────────────────────────────────────────────
  const persistInventory = useCallback(async (updated: Record<string, PerfumeStockItem>) => {
  ...
  }, [selectedPerfume, persistInventory]);
  ...
  const handleAdd = useCallback(() => {
    ...
    setNewName('');
    setAddError('');
  }, [newName, newVersion, inventory, persistInventory]);
    ...
    <>
      <style>{`
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .btn-sync:hover { opacity: 0.9; transform: translateY(-1px); box-shadow: 0 4px 12px rgba(16,185,129,0.25); }
        ...
        .btn-toggle-deactivate:hover { background: rgba(248,113,113,0.18); }
        .btn-toggle-activate {
          background: rgba(52,211,153,0.06);
          color: #34d399;
          border-color: rgba(52,211,153,0.2);
        }
        .btn-toggle-activate:hover { background: rgba(52,211,153,0.16); }
        ...
        <main className="admin-main">
          ...
          <div className="toolbar">
            ...
            <button
              ...
              title="Importar datos del CSV al inventario"
            >
              {importStatus === 'importing' ? (
                <><span className="btn-sync-spinner" /> Importando...</>
              ) : (
                <>📥 Sync CSV</>
              )}
            </button>
          ...
          <div className="table-wrap">
            <table className="inv-table" aria-label="Inventario de perfumes">
              <thead>
                <tr>
                  <th>#</th>
                  <th>Nombre</th>
              ...
              <tbody>
                {loading ? (
                    ...
                    <td colSpan={5}>
                      <div className="empty-state">
                        ...
                        <div className="empty-state-text">
                          {query ? `Sin resultados para "${query}"` : 'El inventario está vacío'}
                        </div>
                      </div>
                    </td>
                  </tr>
                  ...
                  filtered.map(([key, item], idx) => {
                    const verClass = item.version === '1.1'
                      ? 'ver-11'
                      : item.version.toUpperCase() === 'ESTANDAR' || item.version.toUpperCase() === 'STANDARD'
                        ? 'ver-standard'
                        : item.version.toUpperCase() === 'LIMITED'
                          ? 'ver-limited'
                          : 'ver-other';
                    ...
                    return (
                      <tr key={key}>
                        ...
                        <td className="td-name" title={key}>{item.name || key}</td>
                        <td>
                          <span className={`td-version ${verClass}`}>{item.version}</span>
                        </td>
                        <td>
            ...
            <div className="modal-content" onClick={e => e.stopPropagation()}>
              <div className="modal-header">
                <div className="modal-title">
                  <span style={{ color: '#a78bfa' }}>✎</span> Editar Referencia
                </div>
